...
type Message struct {
	...
	IsRead     bool   `gorm:"column:is_read;not null;default:false;index:idx_is_read" json:"is_read"` // 是否已读（0 未读，1 已读）
	_          struct {
		ReceiverID int64
		SenderID   int64
		Type       int32
	...
	_ struct{} `gorm:"index:idx_receiver_type_read,priority:1,columns:receiver_id,type,is_read,deleted_at"` // 添加复合索引
}
