...
type QuestionRepository interface {
	...
	GetQuestions(ctx context.Context, ids []int64) ([]*model.Question, int64, error)
...
func (r *QuestionRepositoryImpl) GetQuestions(ctx context.Context, ids []int64) ([]*model.Question, int64, error) {
	var questions []*model.Question
	var total int64
	err := r.DB.WithContext(ctx).Where("bank_id IN (?)", ids).Find(&questions).Count(&total).Error
